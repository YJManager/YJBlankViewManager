...
@interface UIScrollView () <UIGestureRecognizerDelegate, YJBlankViewDelegate>

@property (nonatomic, readonly) YJBlankView *emptyDataView;

@end

@implementation UIScrollView (YJBlankView)
...
- (void)setEmptyDataView:(YJBlankView *)emptyDataView{
    objc_setAssociatedObject(self, kEmptyDataView, emptyDataView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (YJBlankView *)emptyDataView{
    
    YJBlankView *view = objc_getAssociatedObject(self, kEmptyDataView);
    if (view == nil){
        view = [YJBlankView new];
...
- (void)_reloadEmptyView{
    ...
    if ([self shouldDisplay] && [self _itemsCount] == 0){
        [self _willAppear];
        YJBlankView *view = self.emptyDataView;
        ...
        if (customView) {
            ...
            view.verticalAutoMargin = [self verticalSpace];
        ...
        [view installBlankViewConstraints];
...
- (void)blankView:(YJBlankView *)blankView didClickButton:(UIButton *)button{
    if (self.emptyDelegate && [self.emptyDelegate respondsToSelector:@selector(emptyViewInView:didClickButton:)]) {
        [self.emptyDelegate emptyViewInView:self didClickButton:button];
    }
}
